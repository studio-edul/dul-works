...
import { extractText, extractDate, findProperty, extractRelation, findTitleProperty } from './notion-utils.js';
  ...
  for (const block of blocks) {
    if (block.type === 'image') {
      ...
      if (imageUrl) {
        ...
        images.push({
          filename: filename,
          path: imageUrl, // getExhibitionBySlug에서 로컬 경로로 변환
...
async function extractRelatedTextFromProperties(properties) {
  ...
  if (!relatedTextProperty || relatedTextProperty.type !== 'relation') {
    console.log(`[extractRelatedTextFromProperties] Related Text 속성을 찾을 수 없거나 relation 타입이 아닙니다.`);
    return [];
  }

  const pageIds = extractRelation(relatedTextProperty);
  console.log(`[extractRelatedTextFromProperties] 발견된 관련 페이지 ID 개수: ${pageIds.length}`);
    ...
    try {
      ...
      if (pageInfo && pageInfo.properties) {
        ...
        const titleProperty = findTitleProperty(pageInfo.properties) ||
          findProperty(pageInfo.properties, 'title', 'Title', 'TITLE', 'Name', 'name', 'NAME');

        const title = extractText(titleProperty) || 'Untitled';

        console.log(`[extractRelatedTextFromProperties] ✓ 관련 페이지 발견: "${title}" (${pageId})`);
  ...
  try {
    ...
    if (images.length === 0) {
      ...
      images = images.map(img => ({
        ...img,
        path: `/assets/images/${img.filename}`
      }));
    }
