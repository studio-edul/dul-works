  ...
  const relatedTexts = await Promise.all(pageIds.map(async (pageId) => {
    try {
      ...
      if (pageInfo && pageInfo.properties) {
        ...
        // 디버깅: 속성 값 확인
        // console.log(`[RelatedItem Debug] Title: ${title}, Content Raw: "${contentValue}", Link Raw: "${link}"`);
        // if (pageInfo.properties) {
        //   console.log(`[RelatedItem Debug] Available Properties: ${Object.keys(pageInfo.properties).join(', ')}`);
        // }

        if (normalizedContentValue === 'link' && !link) {
          try {
            // 성능을 위해 첫 5개 블록만 확인
            const blocks = await getPageBlocksServer(pageId);
        ...
        if (normalizedContentValue === 'link') {
          finalUrl = effectiveLink;
        } else if (normalizedContentValue === 'text') {
          finalUrl = null;
        } else {
          // Fallback
          finalUrl = effectiveLink;
