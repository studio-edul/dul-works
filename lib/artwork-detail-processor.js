...
export async function getArtworkBySlug(slug) {
  try {
    ...
    if (images.length === 0) {
      console.log('[DEBUG] No block images found. Using fallback.');
      images = detail.images;
    } else {
      console.log(`[DEBUG] Found ${images.length} block images.`);
    ...
    // 디버깅: 이미지들의 column 정보 확인
    images.forEach((img, idx) => {
      if (idx < 5) console.log(`[DEBUG] Image ${idx}: ${img.filename}, Col: ${img.column}`);
    });
    ...
    console.log(`[DEBUG] PageText length: ${pageText ? pageText.length : 0}`);
    if (pageText && pageText.length > 0) {
      console.log(`[DEBUG] First paragraph:`, JSON.stringify(pageText[0]));
    }
